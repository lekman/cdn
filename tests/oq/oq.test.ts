...
describe(`OQ: resource group (${env})`, () => {
  test("provisioning state is Succeeded", async () => {
...
describe(`OQ: Cosmos DB account (${env})`, () => {
  ...
  test("document endpoint is reachable", async () => {
...
describe(`OQ: APIM CDN API route (${env})`, () => {
  test("GET /cdn/v1/images/{hash} route exists (not 404)", async () => {
